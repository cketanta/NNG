...
**日期**：2026-08-03
**状态**：内容大幅扩充与优化——天赋树主题化扩展（每把 42→38 节点）、敌人体系扩展（2 新小怪 + 5 独立精英 + 2 新 BOSS）、道具扩展至 50 种、难度上调 + BOSS 专属技能、商店价格递增、怪物视觉/图鉴/波次机制/天赋 UI 优化。

**包含内容**：
- **天赋树主题化**：每把武器天赋全部符合自身特点（分裂者移除剧毒、改分裂主题）；每把补足到 42 节点后删繁复线性分支至 38（天赋界面紧凑节点、无滚动、完整显示）
- **敌人体系**：新增 孢子怪（死亡爆裂范围伤害）/ 冰霜射手（远程减速弹）；**5 种独立精英**（狂战士/死灵法师/巨盾者/疾风刺客/毒巫医，第 3 波起概率出现）；**2 新 BOSS**（恶魔领主烈焰范围 / 寒冰女皇冰霜领域，共 7 种 BOSS 每 5 波轮换）
- **道具扩展**：14 → **50 种**（穿甲/燃烧/流血/暴击/吸血/闪避/回血/经验/金币/全武器加成 + 恶魔契约、天使之翼唯一特殊），36 张新图标
- **玩家新状态**：减速（移速减半）/ 中毒（每秒掉血）/ 破甲（受击增伤）
- **难度上调**：刷怪更密更快、怪物血量成长加快、精英概率提升；**BOSS 专属技能强化**（巨魔冲锋、多头蛇更密弹幕、荆棘兽更强脉冲、巫妖更多召唤、蠕虫王分裂 4 只、恶魔领主更多火弹、寒冰女皇更快冰弹）
- **商店**：道具价格随已有个数递增（稀有度越高涨幅越快：普通 15% / 优质 30% / 史诗 50% / 传说 80%）
- **波次机制**：每波 25s → **50s**；**BOSS 波（每 5 波）BOSS 存活时波次不结束**，击杀后才进商店
- **怪物视觉**：8 种小怪不同形状（三角/菱形/六角/尖刺）+ idle 呼吸动画；**BOSS 多结构绘制**（身体 + 金冠 + 各 kind 专属部件 + 旋转动画）；精英分形状
- **图鉴系统**：主界面角落「图鉴」按钮 → 全屏图鉴（怪物 20 种 / 武器 8 把 / 道具 50 种 / 天赋树 4 页签）
- **天赋 UI**：节点缩小、去滚动容器、删繁复分支后完整显示

**关键文件**（回退时对照）：
- `scripts/systems/talent_tree.gd`（主题化节点）、`item_defs.gd`（50 道具 + 价格涨幅）
- `scripts/enemies/`（enemy_spore/frost/elite/boss 新增强化 + enemy_base 形状/状态）
- `scripts/spawner.gd`（8 小怪权重/独立精英/7 BOSS 轮换/难度）、`scripts/main.gd`（波时长/价格/图鉴）
- `scripts/player.gd`（50 道具效果 + 减速/中毒）、`scripts/weapons/*`、`scripts/attacks/*`
- `scripts/ui/bestiary_panel.gd`（新增图鉴）、`difficulty_panel.gd`（图鉴入口）
- `assets/items/*`（50 图标）、`tests/`（boss_test 更新、talent_count ≥32）

**导出**：`exports/v1.5.0/NNG_Ver1.5.0.exe`（单文件，`embed_pck=true`）+ `readme.txt`。

**备注**：25 个无头测试全过。遗留孤儿脚本保留未删。
